...
import "./datachildren.scss";

const Datachildren = ({type,userId}) => {
  
 
  const [List, setList] = useState();
  const {data,error,loading}= useFetch(`http://localhost:5000/api/Parent/childParent/${userId}`);
  console.log(loading);
   useEffect(() => {
    setList(data);
    }, [data]);
  ...
  const handleDelete=async (id)=>{
    try{
      ...
      await axios.delete(`http://localhost:5000/api/Children/${userId}`)
  ...
  return (
    <div className="datatable">
     ...
     { List && <>  <div className="datatableTitle">
        Add New User
        <Link to={`/Children/new`} className="link">
          Add New
        </Link>
      </div>
